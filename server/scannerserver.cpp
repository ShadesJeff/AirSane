...
ScannerServer::ScannerServer(std::shared_ptr<Scanner> pScanner,
...
void
ScannerServer::onRequest(const HttpServer::Request& request,
                         HttpServer::Response& response)
{
  if (request.uri().empty() || request.uri() == "/") {
    ...
    ScannerPage(*mpScanner.get())
      .setTitle(mpScanner->publishedName() + " on " + mHost)
      .render(request, response);
    return;
  }
  if (request.uri() == "/ScannerIcon" &&
      request.method() == HttpServer::HTTP_GET) {
    ...
    if (file.is_open()) {
      ...
      std::clog << "could not open " << mpScanner->iconFile() << " for reading"
                << std::endl;
  ...
  if (request.uri().find(escl) == 0) {
    ...
    if (uriRemainder == "/ScannerCapabilities" &&
        request.method() == HttpServer::HTTP_GET) {
      response.setStatus(HttpServer::HTTP_OK);
    ...
    if (uriRemainder == ScanJobsDir &&
        request.method() == HttpServer::HTTP_POST) {
      bool autoselectFormat = clientIsAirscan(request);
      std::shared_ptr<ScanJob> job = mpScanner->createJobFromScanSettingsXml(
    ...
    res = res.substr(1);
    size_t pos = res.find('/');
    if (pos > res.length() && request.method() == HttpServer::HTTP_DELETE &&
        mpScanner->cancelJob(res)) {
      response.setStatus(HttpServer::HTTP_OK);
      response.send();
      return;
    }
    if (res.substr(pos) == "/NextDocument" &&
        request.method() == HttpServer::HTTP_GET) {
